"""Fake clients that never touch the network: `FakeClient` (sync templates),
`FakeAsyncClient` (a `Bot`). Neither is imported by `__init__.py`; see docs/framework.md.
"""
...
class FakeAsyncClient(AsyncClient):
    """The async counterpart of `FakeClient`, for testing `Bot` end to end; see docs/framework.md."""
    ...
    def respond(self, method, path, response):
        """See `FakeClient.respond` - identical contract, async caller."""
        self._responses[(method, path)] = response
